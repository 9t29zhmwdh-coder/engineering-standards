...
- Dependabot (or Renovate) is configured with `github-actions` ecosystem updates enabled, so pinned SHAs are bumped via a normal, reviewable PR on a schedule rather than silently freezing a repository on a stale or later-disclosed-vulnerable action version. This requires a committed `.github/dependabot.yml`, copied from [`../templates/dependabot.yml`](../templates/dependabot.yml). Repository-level security alerts are not a substitute: they only fire for disclosed vulnerabilities, so a repository can have alerts enabled, look configured, and still never receive a single version update. A portfolio scan on 2026-07-28 found exactly that in 31 of 36 repositories, with `actions/checkout` pins spread across four versions from v4 to v7.0.1.
