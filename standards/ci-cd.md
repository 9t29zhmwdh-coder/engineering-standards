...
- Dependabot (or Renovate) is configured with `github-actions` ecosystem updates enabled, so pinned SHAs are bumped via a normal, reviewable PR on a schedule rather than silently freezing a repository on a stale or later-disclosed-vulnerable action version. This requires a committed `.github/dependabot.yml`, copied from [`../templates/dependabot.yml`](../templates/dependabot.yml). Group minor and patch updates per ecosystem, but let majors arrive individually (`update-types: ["minor", "patch"]` on the group). A breaking change bundled with a dozen harmless patches blocks all of them and produces a diff nobody can review honestly: on 2026-07-28 a single group PR carried React 18 to 19, Tailwind 3 to 4, recharts 2 to 3 and zustand 4 to 5 alongside a security patch that was needed the same day. Actions are the exception and stay grouped wholesale: they are few, the diff is a handful of SHA lines, and a stale pin is a supply-chain concern rather than a compatibility one. Repository-level security alerts are not a substitute: they only fire for disclosed vulnerabilities, so a repository can have alerts enabled, look configured, and still never receive a single version update. A portfolio scan on 2026-07-28 found exactly that in 31 of 36 repositories, with `actions/checkout` pins spread across four versions from v4 to v7.0.1.
